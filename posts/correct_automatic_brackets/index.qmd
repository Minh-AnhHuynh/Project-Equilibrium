...
anova_results <- suppressWarnings(anova_test(value ~ group, data = data))
...
example, you see `add_xy_position()` used, however that can mess up the order of the brackets. Instead, stick with add_y_position, as the x positions can be already determined in some functions. Here both functions work however. Perhaps rstatix got updated ?
This was documented in this [git issue](https://github.com/kassambara/ggpubr/issues/477#issue-1289858095)
